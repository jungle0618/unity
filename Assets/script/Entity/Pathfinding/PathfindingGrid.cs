...
public class PathfindingGrid : MonoBehaviour
{
    ...
    /// <summary>
    /// 獲取最近的可行走節點（當目標位置不可行走時使用）
    /// </summary>
    public PathfindingNode GetNearestWalkableNode(Vector3 worldPosition, float maxSearchRadius = 3f)
    {
        Vector2Int centerGridPos = GetGridPosition(worldPosition);
        PathfindingNode centerNode = GetNode(centerGridPos);
        ...
        // 計算搜索範圍（網格單位）
        int searchRange = Mathf.CeilToInt(maxSearchRadius / cellSize);
        ...
        for (int radius = 1; radius <= searchRange; radius++)
        {
            for (int dx = -radius; dx <= radius; dx++)
            {
                for (int dy = -radius; dy <= radius; dy++)
                {
                    ...
                    int x = centerGridPos.x + dx;
                    int y = centerGridPos.y + dy;
